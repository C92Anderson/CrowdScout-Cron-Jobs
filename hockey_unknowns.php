...
$mysqli = new mysqli("mysql.crowd-scout.net", "ca_elo_games", "cprice31!","nhl_all");
		// Check connection
...
//CEAN UNKNOWNS DATABASE LAST 30 DAYS
 $delete = $mysqli->query("DELETE FROM hockey_unknowns WHERE DATEDIFF (NOW() , game_ts) > 30");
...
WHERE result =  'Unknown'
	AND game_num not in (select distinct game_num from hockey_unknowns)
...
if ($new_unknowns->num_rows > 0) {
						...
						$insert = $mysqli->query("INSERT INTO  `nhl_all`.`hockey_unknowns` (
						`game_num`,
						`user_id` ,
						`game_ts` ,
						`player_id1` ,
